...
#include "AutoLight.cginc"
		...
		{
			float2 uv : TEXCOORD0;
			LIGHTING_COORDS(5, 6) // NEEDED FOR SHADOWS.
		...
		fixed4 frag(v2f i) : SV_Target
		{
			...
			//				float3 normal = i.tangentDir * normalHeight.x + i.normal * normalHeight.z + i.bitangentDir * normalHeight.y;
							...
							for (int kk = 0; kk < 4; ++kk) {
								float4 lightColork = unity_LightColor[kk];
								float3 lightDirectionk = unity_LightPosition[kk].xyz - i.worldPos * unity_LightPosition[kk].w;
		...
		half4 frag_shadow(v2f_shadow IN) : SV_Target
		{
			...
			return saturate((0.1 + half4(saturate(atten), saturate(atten), saturate(atten), 1)));
		}
